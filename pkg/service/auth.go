...
func (s *AuthService) CreateDoctor(doctor smartmed.Doctor) (int, error) {
	doctor.PasswordHash = s.generatePasswordHash(doctor.PasswordHash)
	return s.repo.CreateDoctor(doctor)
}

func (s *AuthService) GenerateToken(email, password string) (string, error) {
	user, err := s.repo.GetDoctor(email, s.generatePasswordHash(password))
	// logrus.Infof("email:%s pass:%s", user.Email, user.PasswordHash)
	if err != nil {
		return "", err
	}
